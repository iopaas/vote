...
public class DateUtil {
	...
	public static LocalDateTime monthStart(int offset) {
		return LocalDateTime.now().plusMonths(offset).with(TemporalAdjusters.firstDayOfMonth()).minusDays(0);
	}
	...
	public static LocalDateTime monthEnd(int offset) {
		return LocalDateTime.now().plusMonths(offset).with(TemporalAdjusters.lastDayOfMonth()).minusDays(0);
	}

	/**
	 * 比較時間当前时间在开始时间和结束时间之间
	 *
	 * @param startTime 開始時間
	 * @param endTime   結束時間
	 * @return
	 */
	public static boolean dateExpire(String startTime, String endTime) {

		try {
			DateTimeFormatter df = DateTimeFormatter.ofPattern(ISO_DATE_TIME_FORMAT);
			long nowTime = LocalDateTime.now().toEpochSecond(ZoneOffset.of("+8"));
			long sdTime = LocalDateTime.parse(startTime, df).toEpochSecond(ZoneOffset.of("+8"));
			long edTime = LocalDateTime.parse(endTime, df).toEpochSecond(ZoneOffset.of("+8"));
			if (nowTime < sdTime || nowTime > edTime) {
				return false;
			}
			return true;
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		System.out.println(LocalDateTime.now().format(DateTimeFormatter.ofPattern(DateUtil.ISO_DATE_FORMAT)));
	}
}
