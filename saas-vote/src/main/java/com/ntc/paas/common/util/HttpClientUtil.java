...
public enum HttpClientUtil {
	...
	private final RequestConfig config = RequestConfig.custom().setSocketTimeout(SOCKET_TIMEOUT)
			.setConnectTimeout(CONNECT_TIMEOUT).setConnectionRequestTimeout(CONN_MANAGER_TIMEOUT).build();
	...
	HttpClientUtil() {
		try {
			...
			logger.error("HttpClientUtil error : " + e.getMessage());
		}
	}

	/**
	 * 避免特殊字符，請使用 URLEncoder.encode（content,"utf-8"）進行轉換 JSON,XML 等 格式傳遞參數 自定義header
	...
	/**
	 * header由調用者set Create by : 2015年9月14日 上午11:54:59
	 */
	...
	/**
	 * 避免特殊字符，請使用 URLEncoder.encode（content,"utf-8"）進行轉換 JSON格式傳遞參數 自定義header Create
	 * by : 2015年9月2日 下午2:52:24
	 */
	...
	/**
	 * 避免特殊字符，請使用 URLEncoder.encode（content,"utf-8"）進行轉換 URL參數map 自定義header Create
	 * by : 2015年9月14日 上午11:02:05
	 */
	public String httpPost(String httpUrl, Map<String, String> paramMap, Map<String, String> headerMap)
			throws Exception {
		try {
	...
	private void setParam(HttpPost httpPost, Map<String, String> paramMap) throws UnsupportedEncodingException {
		if ((null != paramMap) && !paramMap.isEmpty()) {
			...
			for (Map.Entry<String, String> entry : paramMap.entrySet())
				params.add(new BasicNameValuePair(StringUtils.trimToEmpty(entry.getKey()),
